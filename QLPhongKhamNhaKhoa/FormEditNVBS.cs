...
{
    public partial class FormEditNVBS : Form
    {
        public FormEditNVBS()
        {
            InitializeComponent();
        ...
        private void FormEditNVBS_Load(object sender, EventArgs e)
        {
        ...
        private void btn_Edit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DentalClinic_DatabaseEntities db = new DentalClinic_DatabaseEntities())
                {
                    ...
                    if (staff != null)
                    {
                        staff.fullName = txtTenNVBS.Text.Trim();
                        staff.identityNumber = txtCMT.Text.Trim();
                        staff.dateOfBirth = dtpkNgaySinh.Value;
                        staff.gender = radioButton_Nam.Checked;
                        staff.address = txtDiaChi.Text.Trim();
                        staff.phone = txtSoDienThoai.Text.Trim();
                        staff.experience = txtKinhNghiem.Text.Trim();
                        staff.email = txtEmail.Text.Trim();
                        staff.certificateId = txtBangCap.Text.Trim();
                        staff.PositionId = staff.PositionId = int.Parse(cbViTri.SelectedValue.ToString());
                        staff.note = txtGhiChu.Text.Trim();

                        db.SaveChanges();

                        MessageBox.Show("Thông tin bác sĩ đã được cập nhật thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);

                        LoadStaff();
                    }
                    else
                    {
                        MessageBox.Show("Không tìm thấy bác sĩ với mã nhân viên này!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            catch (Exception ex)
